...
def add_exchange_key(key_data: schemas.ExchangeKeyCreate, current_user: models.User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    if existing_key:
        existing_key.api_key_encrypted = fake_encrypt(key_data.api_key)
        existing_key.api_secret_encrypted = fake_encrypt(key_data.api_secret)
        db_key = existing_key
    else:
        db_key = models.ExchangeKey(
            user_id=current_user.id,
            exchange_name=key_data.exchange_name,
            api_key_encrypted=fake_encrypt(key_data.api_key),
            api_secret_encrypted=fake_encrypt(key_data.api_secret)
        )
        db.add(db_key)
...
def get_exchange_keys(current_user: models.User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    results = []
    for k in keys:
        decrypted_key = fake_encrypt(k.api_key_encrypted) 
