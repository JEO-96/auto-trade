...
class MomentumBreakoutEliteStrategy(BaseStrategy):
    ...
    def get_trigger_signals(
        ...
        # 신호 1: CORE BREAKOUT (RSI/MACD/Vol/EMA100, no ADX)
        if all(v is not None for v in (rsi_val, macd_val, macds_val, vol, vol_avg, ema_100)) and vol_avg > 0:
            ...
            is_met = rsi_ok and macd_ok and vol_ok and ema_ok
        ...
        if all(v is not None for v in (ema_200, ema_50, rsi_val)):
            ...
            if prev_low is not None:
                ...
                triggers.append((f"    가격>EMA200: {curr_price:,.0f} vs {ema_200:,.0f}", bool(ema200_ok)))
