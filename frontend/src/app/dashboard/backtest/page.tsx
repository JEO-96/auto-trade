        ...
        return (
            <PageContainer>
                ...
                {shareModal && typeof document !== 'undefined' && createPortal(
                    <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/60 backdrop-blur-sm" onClick={() => setShareModal(null)} role="presentation">
                        <div role="dialog" aria-modal="true" aria-labelledby="share-modal-title-detail" className="rounded-2xl p-6 w-full max-w-md mx-4 bg-th-modal border border-th-border shadow-2xl" onClick={(e) => e.stopPropagation()}>
                            <div className="flex items-center justify-between mb-4">
                                <h3 id="share-modal-title-detail" className="text-base font-bold text-th-text">커뮤니티에 공유</h3>
                                <button onClick={() => setShareModal(null)} aria-label="닫기" className="text-th-text-muted hover:text-th-text transition-colors"><X className="w-4 h-4" /></button>
                            </div>
                            {shareSuccess ? (
                ...
                <div className="flex flex-col gap-5">
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                        <div className="glass-panel p-5 rounded-2xl">
                            <p className="text-[11px] sm:text-xs text-th-text-muted font-medium uppercase tracking-wider mb-2">누적 수익</p>
                        ...
                        <div className="glass-panel p-5 rounded-2xl">
                            <p className="text-[11px] sm:text-xs text-th-text-muted font-medium uppercase tracking-wider mb-2">초기 투자금</p>
                            <p className="text-2xl font-bold text-th-text truncate">₩{Math.round(result.initial_capital).toLocaleString()}</p>
                        </div>

                        <div className="glass-panel p-5 rounded-2xl">
                            <p className="text-[11px] sm:text-xs text-th-text-muted font-medium uppercase tracking-wider mb-2">최종 자산</p>
                            <p className="text-2xl font-bold text-th-text truncate">₩{Math.round(result.final_capital).toLocaleString()}</p>
                        </div>

                        <div className="glass-panel p-5 rounded-2xl">
                            <p className="text-[11px] sm:text-xs text-th-text-muted font-medium uppercase tracking-wider mb-2">매매 횟수</p>
                        ...
                        <div className="grid grid-cols-1 xl:grid-cols-2 gap-5">
                            ...
                            <div className="glass-panel p-6 rounded-2xl">
                                <div className="flex justify-between items-center mb-4">
                                    ...
                                    <span className="text-[10px] sm:text-xs text-th-text-muted">변동률 (%) 기준</span>
                                </div>
                    ...
                    <div className="glass-panel rounded-2xl overflow-hidden">
                        ...
                        <div className="overflow-x-auto">
                            <table className="w-full">
                                <thead>
                                    <tr className="text-[10px] sm:text-xs font-semibold uppercase tracking-wider text-th-text-muted border-b border-th-border-light">
                                ...
                                <tbody className="divide-y divide-th-border-light">
                                    {result.trades.map((trade: BacktestTrade, idx: number) => (
                                        <tr key={idx} className="hover:bg-th-card transition-colors text-sm">
                                            <td className="px-5 py-4">
                                                ...
                                                <p className="text-[10px] sm:text-xs text-th-text-muted mt-0.5">
                                            ...
                                            <td className="px-5 py-4 text-right whitespace-nowrap">
                                                ...
                                                <p className="text-[10px] sm:text-xs text-th-text-muted font-mono mt-0.5">
    ...
    return (
        <PageContainer>
            ...
            {saveStrategyModal && typeof document !== 'undefined' && createPortal(
                <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/60 backdrop-blur-sm" onClick={() => setSaveStrategyModal(null)} role="presentation">
                    <div role="dialog" aria-modal="true" aria-labelledby="save-strategy-title" className="rounded-2xl p-6 w-full max-w-md mx-4 bg-th-modal border border-th-border shadow-2xl" onClick={(e) => e.stopPropagation()}>
                        <div className="flex items-center justify-between mb-4">
                            <h3 id="save-strategy-title" className="text-base font-bold text-th-text">내 전략으로 저장</h3>
                            <button onClick={() => setSaveStrategyModal(null)} aria-label="닫기" className="text-th-text-muted hover:text-th-text transition-colors"><X className="w-4 h-4" /></button>
                        </div>
                        <div className="space-y-3">
                            <input
                                ...
                                maxLength={50}
                                className="w-full px-3 py-2 rounded-lg bg-th-input border border-th-border text-th-text text-sm focus:outline-none focus:border-emerald-400/50"
                            />
                            <p className="text-xs text-th-text-muted">저장된 전략은 봇 생성 시 선택할 수 있습니다. (최대 10개)</p>
            ...
            {shareModal && typeof document !== 'undefined' && createPortal(
                <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/60 backdrop-blur-sm" onClick={() => setShareModal(null)} role="presentation">
                    <div role="dialog" aria-modal="true" aria-labelledby="share-modal-title-list" className="rounded-2xl p-6 w-full max-w-md mx-4 bg-th-modal border border-th-border shadow-2xl" onClick={(e) => e.stopPropagation()}>
                        <div className="flex items-center justify-between mb-4">
                            <h3 id="share-modal-title-list" className="text-base font-bold text-th-text">커뮤니티에 공유</h3>
                            <button onClick={() => setShareModal(null)} aria-label="닫기" className="text-th-text-muted hover:text-th-text transition-colors"><X className="w-4 h-4" /></button>
                        </div>
                        {shareSuccess ? (
                            <div className="text-center py-8">
                ...
                <div className="glass-panel rounded-2xl p-6">
                    ...
                    {historyLoading ? (
                        ...
                        <EmptyState
                            ...
                            title="백테스트 기록이 없습니다"
                            description="위에서 전략을 선택하고 백테스트를 실행해보세요."
                        />
                    ) : (
                                ...
                                return (
                                    ...
                                    >
                                        <div className="p-4 group">
                                            ...
                                            <div className="flex items-center gap-3 text-[11px] sm:text-xs text-th-text-muted flex-wrap mb-3">
                                            ...
                                            {h.status === 'completed' && pnlPct !== null ? (
                                                <div className="flex items-end justify-between">
                                                    ...
                                                    <div className="text-right space-y-0.5">
                                                        <p className="text-[11px] sm:text-xs text-th-text-secondary">
                                        ...
                                        <div className="flex items-center justify-end gap-1 px-3 py-2 border-t border-th-border-light" onClick={(e) => e.stopPropagation()}>
                                            {h.status === 'completed' && (
                                                <>
                                                    <button
                                                        ...
                                                        className="px-3 py-1.5 rounded-lg text-[11px] sm:text-xs font-semibold text-th-text-secondary hover:text-th-text hover:bg-th-card transition-colors"
                                                    >
                                                        상세보기
                                                    </button>
                                                    <button
                                                        ...
                                                        className="px-3 py-1.5 rounded-lg text-[11px] sm:text-xs font-semibold text-primary hover:bg-primary/10 transition-colors flex items-center gap-1"
                                                    >
                                            ...
                                            <button
                                                ...
                                                className="px-2 py-1.5 rounded-lg text-[11px] sm:text-xs text-th-text-muted hover:text-red-400 hover:bg-red-400/10 transition-colors"
                                                title="삭제"
                                            >
                        ...
                        <form onSubmit={runBacktest} className="space-y-6">
                            ...
                            <div>
                                ...
                                <div className="grid grid-cols-2 gap-3">
                                    <div>
                                        <label className="text-[10px] sm:text-xs text-th-text-muted font-medium mb-1 block">시작일</label>
                                        <input
                                            type="date"
                                            name="start_date"
                                    ...
                                    <div>
                                        <label className="text-[10px] sm:text-xs text-th-text-muted font-medium mb-1 block">종료일</label>
                                        <input
                                            type="date"
                                            name="end_date"
                                            value={form.end_date}
                                ...
                                {form.start_date && form.end_date && (
                                    <p className="text-[10px] sm:text-xs text-th-text-muted text-center mt-2">
                                        {form.start_date} ~ {form.end_date}
    ...
    return (
        <div className="relative w-full h-full">
            ...
            {hoverIndex !== null && (
                ...
                >
                    <p className="text-[10px] sm:text-xs text-th-text-muted mb-1">
