    ...
    return (
        <div className="glass-panel glass-panel-hover rounded-2xl p-5 flex flex-col gap-3">
            ...
            {post.post_type === 'backtest_share' && post.backtest_data && (
                <div className="p-3 bg-th-card rounded-xl border border-th-border-light space-y-2">
                    ...
                    {(post.backtest_data.start_date || post.backtest_data.timeframe || post.backtest_data.commission_rate != null) && (
                        <div className="flex items-center gap-3 text-[10px] text-th-text-muted flex-wrap">
            ...
            {post.post_type === 'performance_share' && post.performance_data && (
                <div className="grid grid-cols-3 gap-3 p-3 bg-th-card rounded-xl border border-th-border-light">
                    <MetricItem
                        label="총 PnL"
            ...
            <div className="flex items-center justify-between pt-2 border-t border-th-border-light">
                <div className="flex items-center gap-3">
                    <span className="text-[11px] text-th-text-muted font-medium">
                        {post.author_nickname ?? '익명'}
                    </span>
                    <span className="text-[10px] text-th-text-muted">{timeAgo}</span>
                </div>
                <div className="flex items-center gap-3">
                    <button
...
function MetricItem({ label, value, positive }: { label: string; value: string; positive?: boolean }) {
    return (
        <div className="min-w-0">
            <p className="text-[10px] text-th-text-muted mb-0.5">{label}</p>
        ...
        <>
            <header className="mb-6 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div>
                    ...
                    <p className="text-sm text-th-text-muted">전략 공유, 백테스트 결과, 수익률을 공유하세요</p>
                </div>
                    ...
                    <button
                        ...
                        className={`flex items-center gap-1.5 px-3.5 py-2 rounded-lg text-xs font-semibold whitespace-nowrap transition-colors ${
                            activeTab === tab.value
                                ...
                                : 'text-th-text-muted hover:text-th-text hover:bg-th-hover border border-transparent'
                        }`}
                    >
                        {tab.icon}
                        {tab.label}
                    ...
                    {totalPages > 1 && (
                        <div className="flex items-center justify-center gap-2">
                            <button
                                ...
                                disabled={page === 1}
                                className="p-2 rounded-lg text-th-text-muted hover:text-th-text hover:bg-th-hover disabled:opacity-30 disabled:cursor-not-allowed transition-colors"
                            >
