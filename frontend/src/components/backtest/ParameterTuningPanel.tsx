        ...
        <div className="border border-white/[0.06] rounded-xl overflow-hidden">
            ...
            {enabled && (
                <div className="px-4 pb-4 pt-2 space-y-4 border-t border-white/[0.06]">
                    {/* 청산 모드 */}
                    <div>
                        <label className="text-[10px] sm:text-xs text-th-text-muted font-medium mb-2 block uppercase tracking-wider">청산 모드</label>
                    ...
                    <div>
                        <div className="flex items-center justify-between mb-1.5">
                            <label className="text-[10px] sm:text-xs text-th-text-muted font-medium uppercase tracking-wider">
                    ...
                    {!trailing && (
                        <div>
                            <div className="flex items-center justify-between mb-1.5">
                                <label className="text-[10px] sm:text-xs text-th-text-muted font-medium uppercase tracking-wider">익절률 (TP)</label>
                    ...
                    <div className="pt-3 border-t border-white/[0.06] space-y-3">
                        ...
                        {/* ADX 필터 */}
                        <div className={`rounded-lg border transition-colors ${useAdxFilter ? 'border-purple-500/20 bg-purple-500/[0.03]' : 'border-th-border-light bg-white/[0.01]'}`}>
                            <button type="button" onClick={() => onChange({ useAdxFilter: !useAdxFilter })}
                                className="w-full flex items-center justify-between px-3 py-2">
                                <span className={`text-[11px] sm:text-xs font-semibold ${useAdxFilter ? 'text-purple-400' : 'text-th-text-muted'}`}>ADX 추세 필터</span>
                            ...
                            {useAdxFilter && (
                                <div className="px-3 pb-2.5">
                                    <div className="flex justify-between mb-0.5"><span className="text-[9px] text-th-text-muted">추세 강도 기준</span><span className="text-[10px] sm:text-xs font-bold text-purple-400 font-mono">{adxThreshold}</span></div>
                        ...
                        {/* 거래량 필터 */}
                        <div className={`rounded-lg border transition-colors ${useVolumeFilter ? 'border-amber-500/20 bg-amber-500/[0.03]' : 'border-th-border-light bg-white/[0.01]'}`}>
                            <button type="button" onClick={() => onChange({ useVolumeFilter: !useVolumeFilter })}
                                className="w-full flex items-center justify-between px-3 py-2">
                                <span className={`text-[11px] sm:text-xs font-semibold ${useVolumeFilter ? 'text-amber-400' : 'text-th-text-muted'}`}>거래량 필터</span>
                            ...
                            {useVolumeFilter && (
                                <div className="px-3 pb-2.5">
                                    <div className="flex justify-between mb-0.5"><span className="text-[9px] text-th-text-muted">평균 대비 배수</span><span className="text-[10px] sm:text-xs font-bold text-amber-400 font-mono">{volMultiplier.toFixed(1)}x</span></div>
                        ...
                        {/* ATR 기간 */}
                        <div className="px-1">
                            <div className="flex justify-between mb-0.5"><span className="text-[9px] text-th-text-muted">ATR 변동성 기간</span><span className="text-[10px] sm:text-xs font-bold text-th-text-secondary font-mono">{atrPeriod}</span></div>
                    ...
                    <button
                        ...
                        className="text-[10px] sm:text-xs text-th-text-muted hover:text-primary transition-colors underline underline-offset-2"
                    >
                        전략 기본값으로 초기화
                    </button>
                </div>
